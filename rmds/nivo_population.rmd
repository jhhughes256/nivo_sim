...
```{r covplots, echo = F, fig.asp = 1.5}
  ...
  plot.SECR <- plot.SECR + geom_vline(xintercept = range.SECR.M, linetype = "dashed")
...
While the calculated eGFR using CKD-EPI looks like a strange distribution, it
is more realistic than the sampled GFR as the Bajaj data was also calculated
using this equation. This distribution shape is a product of the equation!!!
...
The CKD-EPI equation enforces a hard cap on the GFR according to the patients
age. Doing a simple multivariate distribution does not capture this.
...
A strange part of the CKD-EPI equation is that it has the inherent assumption 
that patients with serum creatinine concentrations below the average value
have an eGFR less than 120 mL/min. This seems a bit strange, but can should be
easily managed by using a truncated distribution on serum creatinine. The point
of truncation would likely need to be different for males and females.
...
# Extract trough data from output
  trough_flat_df <- output_flat_df %>%
    ...
    dplyr::select(ID, time, amt, evid, rate, cmt, DV, IPRED, AUC, TUM, 
      AGE, ALB, BWT, GFR, SEX, ECOG, EPS1) %>%  # select important columns
    dplyr::group_by(ID) %>%  tidyr::nest() %>%  # group by ID (initiate ddply)
    dplyr::mutate(data = purrr::map(data, function(data) {
      dplyr::mutate(data, dIPRED = IPRED - tail(IPRED, 1)) %>%
      dplyr::slice(-dplyr::n())
    })) %>%  # calculate delta AUC (ddply .fun)
    tidyr::unnest() %>%  # ungroup (collect ddply output)
    dplyr::group_by(time) %>%  tidyr::nest() %>%  # group by ID (initiate ddply)
    dplyr::mutate(data = purrr::map(data, function(data) {
      data.frame(
        mean = mean(data$dIPRED, na.rm = T),
        CI90lo = quantile(data$dIPRED, probs = 0.05, na.rm = T),
        CI50lo = quantile(data$dIPRED, probs = 0.25, na.rm = T),
        median = quantile(data$dIPRED, probs = 0.50, na.rm = T),
        CI50hi = quantile(data$dIPRED, probs = 0.75, na.rm = T),
        CI90hi = quantile(data$dIPRED, probs = 0.95, na.rm = T)
      )
    })) %>%
    tidyr::unnest() %>% head(20)

...
# Populaton parameter variability
  ...
  ZTMAX <- omega.block[4, 4]  # ZTMAX (variance) from model                                                  # ZTMAX (variance) from model
...
# Allocate individuals distribution (standard deviation)
  ...
  ETA5 <- rnorm(n, mean = 0, sd = sqrt(ZEMAX))
  ETA6 <- rnorm(n, mean = 0, sd = sqrt(ZEC50))
...
# Subset population data for regimen
  pop.data <- subset(pop.data, 
    select = c(ID, SEX, BWT, AGE, GFR, ALB, ETA1, ETA2, ETA3, ETA4, ETA5, ETA6)
  )
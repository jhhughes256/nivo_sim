...
source("NivolumabPopPKPopulationwithTS.R")
...
# Plot
p <- ggplot()
p <- p + geom_line(data = test.data, aes(x = time, y = IPRED), col = "blue", size = 0.8)
p <- p + geom_line(data = test.data, aes(x = time, y = TUMSLD), col = "red", size = 0.8)

  ...
  # Loop until all doses have been optimised
    ...
    # Loop until successful minimisation
      ...
      # Initial estimates for Bayes parameters
        init_par <- exp(double(4))
        if (run_once) {
          init_par <- init_par*exp(runif(4, min = -0.01, max = 0.01))
        }
      ...
      # Define bayesian estimation function
        ...
        # Define mrgsolve dataset
          output_estim_df <- cbind(input_estim_df, data.frame(
    ...
    # If nearing the final round of optimisation, add an extra row to receive
      if (next_trough == 364)
    ...
    # End loop once a year of optimised dosing is complete
      if (!is.na(dplyr::summarise_at(bayes_df, "DV", last))[1, 1]) break
